...
(function($){
	...
	$.fn.addPlaceholder = function (options) {
		...
		return this.each(function () {
			...
			if ($.support.placeholder || $(this).hasClass('placeholder')) return false;
		...
		function inputPlacehold(el, ph) {
			...
			if ((valueEmpty(el.val()) || el.val() == ph) && !el.is(':focus')) {
				el.val(ph);
		...
		function passPlacehold(el, ph) {
			...
			var span = $('<span/>',{
				...
				css: {
					...
					top:		el.position().top,
					left:		el.position().left + parseInt(el.css('text-indent')),
